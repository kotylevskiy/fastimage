...
func TestGetInfoReader(t *testing.T) {
	cases := []struct {
		File string
		Info Info
	}{
	...
	for _, c := range cases {
		...
		if err != nil {
			t.Errorf("open file(%+v) error: %+v", c.File, err)
			continue
		}
		info, err := GetInfoReader(file)
		file.Close()
		if err != nil {
			t.Errorf("read file(%+v) error: %+v", c.File, err)
			continue
		...
		if got, want := info, c.Info; got != want {
			t.Errorf("get info reader error, file=%+v, got=%+v, want=%+v,", c.File, got, want)
		}
	}
}
